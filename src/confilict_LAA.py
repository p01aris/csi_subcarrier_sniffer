...
class thread_manager(threading.Thread): #The timer class is derived from the class threading.Thread
    ...
    def __init__(self,node,interval):
        ...
        if 'WIFI' in node:
            self.rf_client1=WIFI_client(node,interval);
        elif 'LTE' in node:
            self.rf_client1=LTE_client(node,interval);
        if 'LAA' in node:
            self.rf_client1=LAA_client(node,interval);
        elif 'LASI' in node:
            self.rf_client1=LASI_client(node,interval);
...
class LTE_client():
    def __init__(self,node_name,interval):
        self.transmit_interval= interval;
...
class LASI_client():
    def __init__(self,node_name,interval):
        ...
        self.transmit_time=0.5;
        self.confict_num=0;
        self.use_freq=[1, 2, 3, 4, 5, 6, 7 ,8 ,9];
        self.node_name=node_name;
        self.data_num=0;
    def transmit(self):
        #传输前监听
...
def start_run(node_array:array,execute_time,interval):
    #Creat threads array for get stock info
    thread_array=[];
    print('start thread init\n');
    #Add thread to array
    for nums in node_array:
         thread_temp=thread_manager(nums,interval);
...
if __name__ == '__main__':
    ...
    for i in range(1,11):
        ...
        for j in range(1,i+1):
            ap_num.append('LAA'+str(j));
        air_space =air_ferq();
        start_run(ap_num,10,0.1);#parameter:ap_name,exe_time,interval_time
    print('WIFI:Change');
    for i in range(1,11):
        ap_num=[];
        ap_num.append('LAA1');
